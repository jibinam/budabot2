...
if (preg_match("/^multiloot (.+)$/i", $message, $arr)) {
	...
	if(preg_match("/^([0-9]+)x (.+)$/i", $arr[1], $lewt) || preg_match("/^([0-9]+) (.+)$/i", $arr[1], $lewt)){
		...
		$this->send("The data you entered is not a multiloot. Please check <highlight>/tell <myname> <symbol>help multiloot<end> for the correct syntax.", "priv");
		return;
	}
	...
	if (!$dontadd) {
		...
		$this->send("To add use <symbol>add $num_loot, or <symbol>add 0 to remove yourself", "priv");
	} else {
		...
		$this->send("To add use <symbol>add $itmref, or <symbol>add 0 to remove yourself", "priv");
		$dontadd = 0;
		$itmref = 0;
