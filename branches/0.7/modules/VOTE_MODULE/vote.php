...
if (preg_match("/^vote$/i", $message)) {
	...
	if ($db->numrows() > 0) {
		...
		$msg = Text::makeLink("Vote Listing", $msg);
	} else {
	...
	if (count($sect) == 1) { // Show vote
		...
		if ($db->numrows() <= 0) { $msg = "Couldn't find any votes with this topic.";} 
			...
			foreach ($results as $key => $value) {
				...
				if ($timeleft > 0) {
					$msg .= Text::makeLink($key, "/tell <myname> vote $question$delimiter$key", 'chatcmd') . "(Votes: $value)\n";
				} else {
			...
			if ($timeleft > 0) { // Want this option avaiable for everyone if its run from org/priv chat.
				$msg .= "\n<black>___%<end> ";
				$msg .= Text::makeLink('Remove yourself from this vote', "/tell <myname> vote remove$delimiter$question", 'chatcmd') . "\n";
			}
			...
			$msg .="<tab>" . Text::makeLink('Kill the vote completely', "/tell <myname> vote kill$delimiter$question", 'chatcmd') . "\n";
			if ($timeleft > 0) {
			...
			$msg = Text::makeLink("Vote: $question",$msg);
