...
/*
* $Id: AOChatPacket.class.php,v 1.1 2006/12/08 15:17:54 genesiscl Exp $
*
...
class AOChatPacket {
	...
	function AOChatPacket($dir, $type, $data) {
		...
		if (!$pmap) {
			Logger::log(__FILE__, "Unsupported packet type ($dir, $type)", ERROR);
			return false;
		}

		if ($dir == "in") {
			if (!is_string($data)) {
				Logger::log(__FILE__, "Incorrect argument for incoming packet, expecting a string", ERROR);
				return false;
			}
			...
			for ($i = 0; $i < strlen($pmap["args"]); $i++) {
				...
				switch($sa) {
					...
					default :
						Logger::log(__FILE__, "Unknown argument type! ($sa)", ERROR);
						continue 2;
				}
				$this->args[] = $res;
			}
			...
			for ($i = 0; $i < strlen($pmap["args"]); $i++) {
				...
				if (is_null($it)) {
					Logger::log(__FILE__, "Missing argument for packet", ERROR);
					break;
				}
